...
const CandidateRegistration = () => {
  ...
  useEffect(()=>{
    getNewCandidate();
    console.log("candidateArray in candidate registration page >>>> ", candidateArray);
  }, [])
  ...
  return (
    ...
    <div  className="relative">
      ...
      <div className="relative rounded-lg border border-cyan-400/30 bg-card p-8 max-w-104 min-w-80">
        {fileUrl && (
          <div className='flex flex-col items-center fixed top-48 left-12 w-60 h-fit bg-black rounded-lg p-6 border-2 border-cyan-400'>
            ...
            <div className='w-full overflow-hidden wrap-break-word'>
              <p className='text-lg font-extrabold text-gray-400 mb-3'>
                Name: &nbsp; <span className='text-white'>{candidateForm.name}</span>
              </p>
              <p className='text-lg font-extrabold text-gray-400 mb-3'>
                Address: &nbsp; <span className='text-white'>{candidateForm.address.slice(0, 20)}...</span>
              </p>
              <p className='text-lg font-extrabold text-gray-400 mb-3'>
                Age: &nbsp; <span className='text-white'>{candidateForm.age}</span>
              </p>
            </div>
          </div>
              ...
              {candidateArray.map((el, i)=> (
                <div key={i+1} className='flex flex-col items-center gap-4 mb-6 bg-gray-950 border border-cyan-400/20 p-4 rounded-lg max-w-52'>
                  <div className='w-24 h-24 rounded-full overflow-hidden border-2 border-cyan-400'>
                    <img src={el[3]} alt="Profile Photo" className='w-full h-full' />
                  </div>
                  <div className='text-center'>
                    <p>{el[2]} #{Number(el[0])}</p>
                    <p>{el[1]}</p>
    ...
    <div  className="relative w-full">
      ...
      <div className="relative rounded-lg border border-cyan-400/30 bg-card p-8 w-full">
        <div className='flex flex-col items-center gap-6 mb-8'>
          <h1 className=' text-cyan-400 text-4xl font-bold mb-4'>Create New Candidate</h1>
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex justify-end mt-8'>
            <Button btnName="Authorize Candidate" handleClick={() => setCandidate(candidateForm, fileUrl)} />
          </div>
        </div>
      </div>
    </div>
